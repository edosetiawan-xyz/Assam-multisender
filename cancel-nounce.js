...
import chalk from "chalk"                                                                       import readline from "node:readline"
import { existsSync, writeFileSync, readFileSync, mkdirSync } from "node:fs"
import { resolve, dirname } from "node:path"
import { fileURLToPath } from "node:url"
import ora from "ora"                                                                           import axios from "axios"
import { Table } from "console-table-printer"
import cliProgress from "cli-progress"
import figlet from "figlet"                                                                     import gradient from "gradient-string"
...
const VERSION = "3.1.0"                                                                         const DATA_DIR = resolve(__dirname, ".data")
...
try {                                                                                             if (!existsSync(DATA_DIR)) {
    mkdirSync(DATA_DIR, { recursive: true })
    console.log(chalk.green(`✓ Direktori data dibuat: ${DATA_DIR}`))                              }
} catch (error) {                                                                                 console.log(chalk.yellow(`⚠ Gagal membuat direktori data: ${error.message}`))                 }
...
const logger = {
  ...
  log: function (level, message, data = null) {                                                     const logEntry = {
    ...
    switch (level) {
      ...
      case "success":
        ...
        break
      case "info":
      default:                                                                                          console.log(chalk.blue(`ℹ️ ${message}`))
    }
  },
  ...
  saveToFile: function () {
    try {
      writeFileSync(LOG_FILE, JSON.stringify(this.logs, null, 2))                                   } catch (error) {
...
function loadEnvConfig() {
  ...
  // Validasi konfigurasi wajib                                                                   const requiredEnvVars = ["RPC_URL"]
  ...
  const hasTelegramConfig = process.env.TELEGRAM_BOT_TOKEN && process.env.TELEGRAM_CHAT_ID        if (!hasTelegramConfig) {
  ...
  if (process.env.PRIVATE_KEYS && process.env.PRIVATE_KEYS.trim() !== "") {
    privateKeys = process.env.PRIVATE_KEYS.split(",")
      .map((key) => key.trim())
      .filter((key) => {                                                                                // Validasi format private key
        try {
  ...
  // Ekstrak token contracts dengan validasi                                                      const tokenContracts = {}
  ...
  return {
    ...
    batchSize: process.env.BATCH_SIZE ? Number.parseInt(process.env.BATCH_SIZE) : PARALLEL_BATCH_SIZE,                                                                                              telegram: {
    ...
    priorityFee: process.env.PRIORITY_FEE ? Number.parseInt(process.env.PRIORITY_FEE) : 1, // GWEI                                                                                                  maxFeePerGas: process.env.MAX_FEE_PER_GAS ? Number.parseInt(process.env.MAX_FEE_PER_GAS) : 0, // 0 = gunakan gasPrice
    ...
    gasPriceCacheDuration: process.env.GAS_PRICE_CACHE_DURATION
      ...
      : 30, // Dalam detik
    retryDelay: process.env.RETRY_DELAY ? Number.parseInt(process.env.RETRY_DELAY) : 2000, // Dalam milidetik                                                                                       concurrentRequests: process.env.CONCURRENT_REQUESTS ? Number.parseInt(process.env.CONCURRENT_REQUESTS) : 3,                                                                                   }
}                                                                                               
...
function loadCache() {
  try {
    ...
    // Buat cache baru jika file rusak                                                              successfulCancellations = new Set()
    saveCache()                                                                                   }
}                                                                                               
...
function saveCache() {                                                                            const now = Date.now()
  ...
  try {
    ...
    writeFileSync(CACHE_FILE, JSON.stringify(data, null, 2))                                        cacheLastSaved = now
  } catch (error) {
...
function loadHistory() {
  ...
  } catch (error) {                                                                                 logger.warning(`Gagal memuat history: ${error.message}`)
...
function saveHistory() {
  ...
  try {
    ...
    if (transactionHistory.length > 1000) {                                                           transactionHistory = transactionHistory.slice(-1000)
    }
...
// Fungsi untuk menambahkan transaksi ke history dengan validasi                                function addToHistory(transaction) {                                                              // Validasi data transaksi
  ...
  // Simpan history secara asinkron                                                               setTimeout(saveHistory, 0)
}

// Fungsi untuk format waktu yang lebih baik                                                    function formatDate(date) {
...
async function processTelegramQueue() {
  ...
  // Tunggu rate limit                                                                            await sleep(TELEGRAM_RATE_LIMIT)
...
function displayBanner() {
  console.log("\n")
  const banner = figlet.textSync("ASSAM TEA", {                                                     font: "Big",
...
function displayStats() {
  if (transactionHistory.length === 0) {
    ...
    return
                                                                                                  ...
                                                                                                  // Hitung statistik                                                                             const totalTx = transactionHistory.length
  ...
  // Hitung total biaya                                                                           const totalCost = transactionHistory                                                              .filter((tx) => tx.status === "success" && tx.cost)
  ...
  console.log(chalk.cyan("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"))             console.log(chalk.green(`✓ Total transaksi: ${totalTx}`))
  console.log(chalk.green(`✓ Berhasil: ${successTx} (${successRate}%)`))
  console.log(chalk.red(`✗ Gagal: ${failedTx}`))                                                  console.log(chalk.yellow(`⚠ Pending: ${pendingTx}`))
  ...
  // Tampilkan 5 transaksi terakhir                                                               console.log(chalk.cyan("5 TRANSAKSI TERAKHIR:"))
...
async function checkTransactionStatus(txHash, blockExplorerUrl, config) {
  // Cek cache dulu                                                                               if (txStatusCache.has(txHash)) {
    return txStatusCache.get(txHash)
  }

  try {
    // Coba dapatkan status transaksi dari block explorer menggunakan API
    const apiUrl = `${blockExplorerUrl.replace("/tx/", "/api/v1/tx/")}${txHash}`                    const response = await axios.get(apiUrl, {                                                        timeout: 5000,
      headers: {
...
async function getOptimalGasPrice(provider, config) {
  ...
  try {
    ...
    const suggestedGasPrice = Math.ceil(Number.parseFloat(ethers.formatUnits(networkGasPrice.gasPrice, "gwei")))                                                                                                                                                                                    // Tentukan faktor pengali berdasarkan strategi
    let multiplier = 1.2 // default (optimal)
    if (config.gasPriceStrategy === "aggressive") {                                                   multiplier = 1.5
    ...
    // Pastikan minimal sesuai faktor pengali dari gas price jaringan                               recommendedGasPrice = Math.max(recommendedGasPrice, suggestedGasPrice * multiplier)
    ...
    lastGasPriceCheck = now
                                                                                                    return cachedGasPrice                                                                         } catch (error) {
...
async function cancelTransaction(wallet, nonce, gasPrice, config, retryCount = 0) {
  ...
  if (config.dryRun) {
    ...
    // Tambahkan ke history                                                                         addToHistory({
  ...
  if (successfulCancellations.has(cacheKey)) {                                                      logger.info(`Nonce ${nonce} sudah berhasil dibatalkan sebelumnya.`)
    return true
  }
  ...
  try {
    ...
    // Gunakan maxFeePerGas dan maxPriorityFeePerGas jika diaktifkan                                if (config.maxFeePerGas > 0) {
      tx = {
        to: wallet.address, // Kirim ke diri sendiri                                                    value: 0n, // 0 TEA
        nonce: nonce,
        maxFeePerGas: ethers.parseUnits(config.maxFeePerGas.toString(), "gwei"),                        maxPriorityFeePerGas: ethers.parseUnits(config.priorityFee.toString(), "gwei"),
      ...
      tx = {
        ...
        value: 0n, // 0 TEA                                                                             nonce: nonce,                                                                                   gasPrice: ethers.parseUnits(gasPrice.toString(), "gwei"),                                       gasLimit: GAS_LIMIT_DEFAULT,                                                                  }
    }

    // Kirim transaksi                                                                              const response = await wallet.sendTransaction(tx)
    spinner.succeed(`Transaksi pembatalan dikirim: ${response.hash}`)                           
    // Tambahkan ke history sebagai pending
    addToHistory({
      wallet: wallet.address,
      nonce: nonce,                                                                                   hash: response.hash,
      status: "pending",                                                                              gasPrice: gasPrice,
      timestamp: new Date().toISOString(),
    })
                                                                                                    // Tunggu konfirmasi                                                                            const waitSpinner = ora(`Menunggu konfirmasi untuk nonce ${nonce}...`).start()
    ...
    if (receipt && receipt.status === 1) {
      ...
      // Hitung biaya transaksi                                                                       const cost = ethers.formatEther(receipt.gasUsed * (tx.gasPrice || receipt.effectiveGasPrice || 0n))                                                                                       
      // Update history
      const historyIndex = transactionHistory.findIndex(                                                (t) => t.wallet === wallet.address && t.nonce === nonce && t.hash === response.hash,
      )

      if (historyIndex >= 0) {
        transactionHistory[historyIndex].status = "success"                                             transactionHistory[historyIndex].blockNumber = receipt.blockNumber
        transactionHistory[historyIndex].gasUsed = receipt.gasUsed.toString()
        transactionHistory[historyIndex].cost = cost                                                    saveHistory()                                                                                 }
      ...
      // Verifikasi dengan block explorer jika URL tersedia                                           if (config.blockExplorer) {
      ...
      if (historyIndex >= 0) {
        transactionHistory[historyIndex].status = "failed"                                              saveHistory()
      }

      // Kirim notifikasi untuk transaksi gagal                                                       await sendTelegramNotification(                                                                   config,
        `<b>Pembatalan Gagal</b>\n\n` +
          `🔢 <b>Nonce:</b> <code>${nonce}</code>\n` +
          `👛 <b>Wallet:</b> <code>${wallet.address}</code>\n` +
          `🧾 <b>Hash:</b> <code>${response.hash}</code>\n` +
          `⛽ <b>Gas Price:</b> <code>${gasPrice} GWEI</code>\n` +                                        `❓ <b>Status:</b> Gagal`,                                                                    response.hash,
        "error",
      )

      return false
    }
  } catch (error) {                                                                                 // Jika error "replacement transaction underpriced", coba lagi dengan gas price lebih tinggi
    if (error.message.includes("replacement transaction underpriced") && retryCount < MAX_RETRY_COUNT) {
      ...
      const retryDelay = config.retryDelay * Math.pow(1.5, retryCount)                                await sleep(retryDelay)
    ...
    if (                                                                                              error.message.includes("nonce has already been used") ||
      ...
      successfulCancellations.add(cacheKey) // Tambahkan ke cache berhasil                            saveCache()                                                                               
    ...
    // Tambahkan ke history
    addToHistory({                                                                                    wallet: wallet.address,
...
async function batchCancelTransactions(wallet, nonces, gasPrice, config) {
  ...
  const progressBar = new cliProgress.SingleBar({
    format: `Pembatalan Batch |${chalk.cyan("{bar}")}| {percentage}% | {value}/{total} Nonces | ⏱️ {eta}s`,                                                                                          barCompleteChar: "\u2588",                                                                      barIncompleteChar: "\u2591",                                                                    hideCursor: true,
  })                                                                                            
  progressBar.start(nonces.length, 0)

  // Proses nonce dalam batch dengan worker pool                                                  const processBatch = async (startIdx) => {                                                        const endIdx = Math.min(startIdx + batchSize, nonces.length)
...
async function getTokenBalance(provider, tokenAddress, walletAddress) {
  try {
    ...
    if (tokenBalanceCache.has(cacheKey)) {
      ...
      // Gunakan cache jika belum expired (5 menit)                                                   if (Date.now() - cachedData.timestamp < 5 * 60 * 1000) {
        return cachedData
      }
    }
                                                                                                    // Buat kontrak
...
async function displayWalletInfo(wallet, provider, index, total, config) {
  ...
  try {
    ...
    if (config.checkTokenBalances && Object.values(config.tokenContracts).some((addr) => addr)) {
      ...
      // Dapatkan saldo untuk setiap token yang dikonfigurasi secara paralel                          const tokenPromises = Object.entries(config.tokenContracts)                                       .filter(([_, addr]) => addr)
...
async function processWallet(wallet, provider, config, gasPrice) {
  try {
    ...
    const maxCost = ethers.formatEther(                                                               BigInt(GAS_LIMIT_DEFAULT) * ethers.parseUnits(gasPrice.toString(), "gwei") * BigInt(pendingTxCount),
    )
...
async function monitorPendingTransactions(config, provider) {
  ...
  await sendTelegramNotification(
    config,                                                                                         `<b>Monitoring Transaksi Dimulai</b>\n\n` +
  ...
  process.removeAllListeners("SIGINT")
    ...
    await sendTelegramNotification(
      ...
      "info",
                                                                                                  ...
                                                                                                  while (isRunning) {
    iteration++                                                                                     const startTime = Date.now()
    console.log(chalk.cyan(`\n[Monitoring #${iteration}] ${new Date().toLocaleString()}`))
      ...
      let totalSuccess = 0
      let totalFail = 0
      let pendingDetected = false                                                                                                                                                                     // Buat array wallet yang akan diproses
      let wallets = []

      // Jika ada private keys, gunakan itu
      if (config.privateKeys.length > 0) {                                                              wallets = config.privateKeys.map((key) => new ethers.Wallet(key, provider))
      }
      // Jika tidak ada private keys tapi ada wallet address, cek saja status                         else if (config.walletAddress) {
        logger.info(`Monitoring wallet address: ${config.walletAddress} (read-only)`)

        try {
          // Dapatkan nonce terbaru                                                                       const currentNonce = await provider.getTransactionCount(config.walletAddress, "latest")
          const pendingNonce = await provider.getTransactionCount(config.walletAddress, "pending")                                                                                              
          logger.info(`Nonce saat ini (latest): ${currentNonce}`)                                         logger.info(`Nonce pending: ${pendingNonce}`)

          if (pendingNonce > currentNonce) {
            logger.warning(`Ditemukan ${pendingNonce - currentNonce} transaksi pending`)
            pendingDetected = true                                                              
            // Kirim notifikasi jika ada transaksi pending                                                  await sendTelegramNotification(
              config,                                                                                         `<b>Transaksi Pending Terdeteksi</b>\n\n` +                                                       `👛 <b>Wallet:</b> <code>${config.walletAddress}</code>\n` +                                    `🔢 <b>Nonce Saat Ini:</b> <code>${currentNonce}</code>\n` +
          ...
          } else {                                                                                          logger.success(`Tidak ada transaksi pending untuk wallet ini.`)                               }
                                                                                                          // Cek saldo token jika diaktifkan                                                              if (config.checkTokenBalances && Object.values(config.tokenContracts).some((addr) => addr)) {
            logger.info(`Memeriksa saldo token...`)

            // Dapatkan saldo untuk setiap token secara paralel                                             const tokenPromises = Object.entries(config.tokenContracts)
              .filter(([_, addr]) => addr)                                                                    .map(async ([tokenKey, tokenAddress]) => {
                try {
                  return {
                    key: tokenKey,
        ...
        } catch (error) {                                                                                 logger.error(`Error saat memeriksa wallet: ${error.message}`)
        }                                                                                             }
      ...
      for (let i = 0; i < wallets.length; i++) {
        ...
        try {
          ...
          // Jika ada transaksi pending, proses                                                           if (pendingNonce > currentNonce) {
            pendingDetected = true

            if (config.autoMode) {                                                                            logger.warning(`Ditemukan ${pendingNonce - currentNonce} transaksi pending, memproses secara otomatis...`)
        ...
        } catch (error) {                                                                                 logger.error(`Error saat memproses wallet: ${error.message}`)
        }
      }
  ...
  // Tunggu user menekan Enter untuk kembali ke menu utama                                        await question(chalk.yellow("Tekan Enter untuk kembali ke menu utama..."))
  return
}
  ...
  const p = new Table({
    columns: [
      ...
      { name: "native", title: NATIVE_TOKEN_SYMBOL, alignment: "right" },
                                                                                                  ...
                                                                                                  // Spinner untuk menunjukkan progres                                                            const spinner = ora("Mengambil data saldo token...").start()                                  
  // Periksa setiap wallet dengan Promise.all untuk paralelisasi
  const walletPromises = addresses.map(async (address) => {                                         try {
      ...
      const tokenPromises = configuredTokens.map(async ([key, tokenAddress]) => {                       try {
      ...
      let rowColor = "green"
      if (balanceInEth < 0.01) rowColor = "red"                                                       else if (balanceInEth < 0.05) rowColor = "yellow"

      return { rowData, rowColor, address, balanceInEth, tokenResults }
    } catch (error) {
      logger.error(`Error saat memeriksa wallet ${address}: ${error.message}`)                        return { error: true, address, message: error.message }
    }
  })
  ...
  for (const result of results) {
    if (!result.error) {
      p.addRow(result.rowData, { color: result.rowColor })
                                                                                                  ...
                                                                                                  // Tampilkan tabel
  p.printTable()                                                                                
  // Kirim notifikasi dengan saldo token                                                          let telegramMessage = `<b>Laporan Saldo Token</b>\n\n`

  for (const result of results) {
    if (result.error) {
      telegramMessage += `👛 <b>Wallet:</b> <code>${result.address}</code>\n`                         telegramMessage += `❌ <b>Error:</b> <code>${result.message}</code>\n\n`
    } else {                                                                                          telegramMessage += `👛 <b>Wallet:</b> <code>${result.address}</code>\n`
...
// Fungsi untuk memproses semua wallet                                                          async function processAllWallets(config, provider, gasPrice) {
  let totalSuccess = 0
  let totalFail = 0
  ...
  for (let i = 0; i < config.privateKeys.length; i++) {
    ...
    try {
      ...
      logger.error(`Error saat memproses wallet: ${error.message}`)                             
      // Kirim notifikasi untuk error wallet
      await sendTelegramNotification(                                                                   config,                                                                                         `<b>Error Pada Wallet</b>\n\n` +
          `👛 <b>Wallet:</b> <code>${wallet.address}</code>\n` +
          `❌ <b>Error:</b> <code>${error.message}</code>`,
        null,
        "error",                                                                                      )
...
// Fungsi untuk memproses satu wallet saja                                                      async function processSingleWallet(config, provider, gasPrice) {                                  console.log(chalk.cyan("\nDaftar wallet tersedia:"))
  for (let i = 0; i < config.privateKeys.length; i++) {
...
async function cancelAllPendingTransactions() {
  ...
  try {                                                                                             // Muat logger, cache dan history
    logger.loadFromFile()                                                                           loadCache()
    loadHistory()
    ...
    let provider                                                                                    let retryCount = 0
    const maxRetries = 3

    while (retryCount < maxRetries) {
      try {
        provider = new ethers.JsonRpcProvider(config.rpcUrl)                                            await provider.getNetwork() // Test koneksi
    ...
    while (true) {
      ...
      if (menuChoice === "6") {
        ...
        break                                                                                         }
                                                                                                      // Jika pilihan 3 (monitoring), jalankan monitoring                                             if (menuChoice === "3") {
      ...
      if (menuChoice === "2") {
        // Mode satu wallet                                                                             await processSingleWallet(config, provider, gasPrice)
